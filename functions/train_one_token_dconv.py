...
def train_one_token_dconv(path_work, model, dataloader_train, device, hp, valid_fn=None, dataloader_valid=None, test_num_pos=0, args=None):
    ...
    for epoch in range(start_epoch, num_epoch):
        ...
        for index, batch in enumerate(dataloader_train):
            ...
            if args.model == 'segmenter':
                ...
                if args.mid_decode:
                    ...
                    # mid_pix_mask = output["mid_pix_mask"]

                    # distill_token = output['one_cls_token']
                    # cnn_cls_token = output['cnn_cls_token']
                    
                    # print("@@@@@@@@@@@@", mid_masks.shape, mid_pix_mask.shape)
                    # exit()
                ...
                if not args.weakly: # 如果不是弱監督的話
                    ...
                    # print(torch.sigmoid(y).shape, label.shape) # [16, 4] # [16, 4]
                    # exit()

                    lossf = loss_fn(torch.sigmoid(y), label)
                    
                    # losst = loss_fn(torch.sigmoid(mid_masks).squeeze(1), torch.sigmoid(cnn_cls_token))
                    
                    if args.cnn_distill:
                        cnn_masks = output["cnn_patch_masks"]
                        # print(cnn_masks.shape)
                        # exit()
                        y_cnn = ngwp_focal(cnn_masks)
                        lossf += loss_fn(torch.sigmoid(y_cnn), label)
                        
                        if epoch >= 1: # distillation
                            alpha = 0.5
        ...
        # torch.save(state, path_work + ckpt_name)
        # torch.save(state, os.path.join(path_work, ckpt_name.split(".")[0] +"_" + str(epoch)+".pth"))
